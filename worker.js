        ...
        if (payout_method && payout_details) {
          await env.DB.prepare(`
            ...
            ON CONFLICT(creator_id) DO UPDATE SET
              ...
              paypal_email=CASE WHEN excluded.method='paypal' THEN excluded.paypal_email ELSE paypal_email END,
              updated_at=excluded.updated_at
          `).bind(user_id, payout_method, payout_method==='paypal'?payout_details:'').run().catch(()=>{});
        }
