 ...
 * Bindings set in CF Dashboard → Workers & Pages → vino-realty → Settings → Bindings:
 *   D1 Database  → Variable name: DB
 *   R2 Bucket    → Variable name: PHOTOS
...
function arr(v) { if(!v) return []; if(Array.isArray(v)) return v; try{return JSON.parse(v);}catch{return [];} }
function makeRef() { return 'VN'+Math.random().toString(36).slice(2,8).toUpperCase(); }
  ...
  async fetch(req, env) {
    try {
      ...
      if (p==='/api/photos' && m==='POST') {
        const { photos=[], prefix='photo' } = await getBody(req);
      ...
      if (p==='/api/properties' && m==='GET') {
        if (!env.DB) return j({ data:[] });
        const { results } = await env.DB.prepare(
          `SELECT * FROM properties WHERE id > 0 ORDER BY boosted DESC, created_at DESC`
        ).all();
      ...
      if (p==='/api/properties' && m==='POST') {
        if (!env.DB) return jErr('DB not bound');
        const d = await getBody(req);
        const photos = await storePhotos(env, arr(d.photos), 'prop');
        const r = await env.DB.prepare(`
          INSERT INTO properties
            (type,title,location,province,price,beds,baths,area_num,area,land,
             land_perches,img,amenities,deed,badge,badge_key,description,photos,
             listing_mode,country,boosted,boosted_until,boosted_days)
          VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
        `).bind(
          ...
          d.description||d.desc||null, JSON.stringify(photos),
      ...
      if (pm) {
        ...
        if (sub==='/boost' && m==='POST') {
          const { days=7 } = await getBody(req);
        ...
        if (m==='PUT') {
          const d = await getBody(req);
          const sets=[], vals=[];
          const direct = ['type','title','location','province','price','beds','baths',
            'area','land','img','deed','badge','listing_mode','country',
            'boosted','boosted_until','boosted_days'];
          for (const k of direct) if (d[k]!==undefined) { sets.push(`${k}=?`); vals.push(d[k]); }
      ...
      if (p==='/api/submissions' && m==='POST') {
        ...
        const r = await env.DB.prepare(`
          INSERT INTO ad_submissions
            (ref_code,submitter_name,submitter_phone,type,title,location,province,
             price,beds,baths,area,land,land_perches,img,amenities,deed,description,
             photos,listing_mode,country,package_name,package_price,payment_ref,
        ...
        `).bind(
          ...
          d.img||'img-custom',
          JSON.stringify(arr(d.amenities)),
          d.deed||'freehold', d.description||null,
      ...
      // ── /api/submissions/:id/approve|reject ────────────────────────────
      ...
      if (p==='/api/users/register' && m==='POST') {
        if (!env.DB) return jErr('DB not bound');
        const { phone, name, password, ref_code } = await getBody(req);
      ...
      return j({ error:{ message: e.message || 'Worker error' } }, 500);
    }
  }
};
